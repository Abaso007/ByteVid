...
def match_summaries_with_timestamps(
    summaries: list[str], subs: list[Sub]
) -> list[Optional[int]]:
    res = []
...
def timestamps_to_images(
    video_path: str, dir_out: str, timestamps: list[Optional[int]]
) -> list[Optional[str]]:
...
def determine_keyframes(work_dir: str, summaries: list[str]) -> list[Optional[str]]:
    ...
    video_file = video_file_orig.replace("video.", "preprocessed.").replace(
        "webm", "mp4"
    )
